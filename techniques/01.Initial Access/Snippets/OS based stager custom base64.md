# OS based stager custom base64

Macro malware that retrieves the OS (Windows or OSX) and executes the appropriate payload

### Execution

1. Detect and send OS and Username base64 encoded to C2
   * Environ$("comutername")
   * Environ$("username")
   * socket.gethostname()
   * os.environ[" & Chr(34) & "USER" & Chr(34) & "] //os.environ["USER"]
2. Network connection via GetHTTPResult (Win) or urllib2 (OSX)
3. Payload exec via PowerShell (Win) or python (OSX)
